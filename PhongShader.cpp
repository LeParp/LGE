...
PhongShader::PhongShader(unsigned lights_count)
{
    ...
    locations_[PROJECTION] = glGetUniformLocation(program_, "Projection");
    locations_[VIEW] = glGetUniformLocation(program_, "View");
    locations_[MODEL] = glGetUniformLocation(program_, "Model");

    locations_[MATERIAL_DIFFUSE_COLOR] = glGetUniformLocation(program_, "Material.diffuse_color");
    locations_[MATERIAL_SPECULAR_COLOR] = glGetUniformLocation(program_, "Material.specular_color");
    locations_[MATERIAL_SHININESS] = glGetUniformLocation(program_, "Material.shininess");
    locations_[MATERIAL_TEXTURED] = glGetUniformLocation(program_, "Material.textured");

    glUniform1i(glGetUniformLocation(program_, "Material.diffuse_texture"), MATERIAL_DIFFUSE);
    glUniform1i(glGetUniformLocation(program_, "Material.specular_texture"), MATERIAL_SPECULAR);

    for(GLuint i = 0; i < lights_count; ++i)
    {
        array_locations_[LIGHT_POSITION].push_back(glGetUniformLocation(program_, ("Lights[" + std::to_string(i) + "].position").c_str()));
        array_locations_[LIGHT_COLOR].push_back(glGetUniformLocation(program_, ("Lights[" + std::to_string(i) + "].color").c_str()));
        array_locations_[LIGHT_ATTENUATIONS].push_back(glGetUniformLocation(program_, ("Lights[" + std::to_string(i) + "].attenuations").c_str()));
    }
}
